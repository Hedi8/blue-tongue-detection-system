        ...
        for classId, confidence,box in zip(classIds.flatten(),confs.flatten(),bbox):
            ...
            if className in objects: 
                ...
                if (draw):
                    ...
                    _, img = cap.read()
                    dataqr, bbbox, _ = detector.detectAndDecode(img)
                    if(bbbox is not None):
                        for i in range(len(bbbox)):
                            cv2.line(img, tuple(bbbox[i][0]), tuple(bbbox[(i+1) % len(bbbox)][0]), color=(255,
                                     0, 0), thickness=2)
                        cv2.putText(img, dataqr, (int(bbbox[0][0][0]), int(bbbox[0][0][1]) - 10), cv2.FONT_HERSHEY_SIMPLEX,
                                    1, (255, 250, 120), 2)
                    ...
                    if(float(targetTemp)>23.0):
                        cv2.imwrite('image'+img_counter+'.jpg', img)
                        storage.child('vache/image'+img_counter+'.jpg').put('image'+img_counter+'.jpg')
                        data={'num':dataqr,'image':'image'+img_counter,'targetTemp':targetTemp,'ambientTemp':ambientTemp}
